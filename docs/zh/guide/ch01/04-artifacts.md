...
- **快速扫描**：使用高螺距（pitch > 1.0）和多排探测器
- **心电门控**：心脏 CT 中同步心电信号，在舒张期采集数据
- **后处理**：运动校正算法

::: warning ⚠️ 临床注意
运动伪影常见于腹部扫描（呼吸运动）和心脏扫描（心跳）。在评估小病灶时，需要区分真实病变和运动伪影。多层面观察和对比增强前后扫描有助于鉴别。
:::
...
- **脂肪抑制技术**：STIR、脂肪饱和（Fat Sat）
- **增加接收带宽**：提高频率分辨率
- **交换编码方向**：将脂肪-水界面置于相位编码方向

::: info 💡 诊断价值
化学位移伪影虽然是伪影，但在某些情况下具有诊断价值。例如，在肾脏脂肪沉积的诊断中，化学位移成像可以用于定量评估脂肪含量。
:::
...
- **序列选择**：使用自旋回波序列（SE）而非梯度回波序列（GRE）
- **参数调整**：
  - 缩短回波时间（TE）
  - 增加接收带宽
  - 使用厚层扫描
- **特殊技术**：WARP、SEMAC、MAVRIC 等金属伪影减除技术

::: tip 💡 临床应用
磁敏感伪影在某些情况下具有诊断价值。在 SWI（磁敏感加权成像）序列中，可以利用磁敏感伪影的特性来检测微出血、静脉血管和铁沉积，这对脑卒中、帕金森病等疾病的诊断具有重要意义。
:::
...
- **患者准备**：呼吸训练、屏气扫描、固定装置、必要时使用镇静剂
- **扫描技术**：
  - 呼吸门控：同步呼吸信号
  - 心电门控：同步心电信号
  - 饱和带：抑制运动区域的信号
  - 快速序列：EPI、HASTE 等
- **编码方向优化**：将相位编码方向设置为垂直于运动方向

::: warning ⚠️ 临床注意
运动伪影是 MRI 中最常见的伪影之一。在腹部和心脏成像中，应优先考虑使用呼吸门控或快速序列来减少运动伪影。
:::
