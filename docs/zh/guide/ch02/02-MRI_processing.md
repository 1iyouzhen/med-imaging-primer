...
$$
s(k_x(t), k_y(t)) = \iint M_0(x,y)e^{-i,2\pi,(k_x,x + k_y,y)} dxdy
$$
...
```
MRI扫描期间采集 k-空间数据 + navigator 位置数据 $Δr(t)$  
    ↓  
估计每条k-空间线\每个 采样时间 $t_i$ 的位移 $Δr(t_i)$  
    ↓  
导航补偿或重排序  
    — 若采用Navigator方式：对 $s(k,t_i)$ 做相位补偿 → $s_corr(k,t_i)$  
    — 若采用回顾式重排序：映射 $k → k+δk_i$　→ 重建 $s_corr(k)$  
    ↓  
逆傅里叶变换 → $I_corr(x,y)$  
    ↓  
